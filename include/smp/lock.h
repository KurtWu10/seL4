...
{
    clh_qnode_t *prev;
    __atomic_store_n(&big_kernel_lock.node_owners[cpu].node->value, CLHState_Pending, __ATOMIC_RELAXED);
...
{
    return __atomic_load_n(&big_kernel_lock.node_owners[getCurrentCPUIndex()].node->value, __ATOMIC_RELAXED) == CLHState_Pending;
}
